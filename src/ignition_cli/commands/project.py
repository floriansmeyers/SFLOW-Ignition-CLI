...
def import_project(
    ...
    with _client(gateway, url, token) as client:
        ...
        client.post(f"/projects/import/{project_name}", content=file_path.read_bytes(), params=params)
...
def resources(
    ...
    with _client(gateway, url, token) as client:
        data = client.get_json(f"/projects/find/{name}")
        items = data.get("resources", data.get("items", [])) if isinstance(data, dict) else data
